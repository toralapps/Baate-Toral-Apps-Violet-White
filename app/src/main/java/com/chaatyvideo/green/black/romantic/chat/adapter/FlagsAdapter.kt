...
class FlagsAdapter(private val interaction: Interaction? = null) :
    ...
    constructor(
        ...
        val startage = itemView.findViewById<TextView>(R.id.agestart)
        val endage = itemView.findViewById<TextView>(R.id.endage)

        fun bind(item: Falgs) = with(itemView) {

            flagIcon.setImageResource(item.flag)
            startage.text = item.startAge
            endage.text = item.endAge
