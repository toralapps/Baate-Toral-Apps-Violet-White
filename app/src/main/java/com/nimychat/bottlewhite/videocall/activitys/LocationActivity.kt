...
import com.nimychat.bottlewhite.videocall.adapter.FlagsAdapter
import com.nimychat.bottlewhite.videocall.extenstionfunctions.snackBar
import com.nimychat.bottlewhite.videocall.livevideocall.activity.ConnectLiveActivity
import com.nimychat.bottlewhite.videocall.repository.Response
import com.nimychat.bottlewhite.videocall.videolistmodel.Falgs
import com.nimychat.bottlewhite.videocall.videolistmodel.SavedVideoList
...
class LocationActivity : NewAddsActivty(), FlagsAdapter.Interaction {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val flags = listOf<Falgs>(
            Falgs(R.drawable.collage_4,"Hookup girl call"),
            Falgs(R.drawable.collage_6,"Wealthy girl call"),
            Falgs(R.drawable.collage_8,"Cute girl call"),
           Falgs(R.drawable.collage_1,"Angry girl call")
        )
    ...
    private fun setUpVideoCallType(videoCall: String, videoList: VideoList) {
        videoCall.let {
            if(it == "Fail"){
                ...
                com.nimychat.bottlewhite.videocall.videolistmodel.SavedVideoList.saveVideos(this,videoList)
                videoCallType = videoCall
            }else if(it == "Live"){
                com.nimychat.bottlewhite.videocall.videolistmodel.SavedVideoList.saveVideos(this,videoList)
                videoCallType = videoCall
            }else if (it == "Cache"){
                val video = com.nimychat.bottlewhite.videocall.videolistmodel.SavedVideoList.getVideos(this)
    ...
    override fun onItemSelected(position: Int, item: Falgs) {
        if(com.nimychat.bottlewhite.videocall.videolistmodel.SavedVideoList.getVideos(this) != null) {
            videoCallType?.let {
                if (it == "Fail") {
                    Toast.makeText(this,
